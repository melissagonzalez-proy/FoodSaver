...
export const ForgotPasswordPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback({ 
        type: "error", 
        message: error.response?.data?.message || "Hubo un error al intentar enviar el correo." 
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-brand-background flex flex-col justify-center items-center p-4 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-brand-card border border-brand-border rounded-4xl p-8 shadow-2xl z-10">
        ...
        <div className="mt-8 text-center">
          <Link to="/login" className="inline-flex items-center gap-2 text-sm text-brand-muted hover:text-brand-text transition-colors">
